...
특수벽은 강철·화염·독·빙결·바람·위상·가시·수정의 8종입니다. 화염벽은 캐릭터를 불태우고 기존에 발견한 벽 기억을 즉시 지운 뒤 4행동 동안 새로 알아낸 벽도 다음 자기 행동 직전에 사라지게 합니다. 불타는 동안 밟은 연막은 소비되지만 불길에 흩어져 화염 지속 시간을 끊지 못합니다. 독벽은 다음 4행동의 모든 방향 입력을 상·하·좌·우 중 하나로 무작위 변환합니다. 빙결·바람의 강제 이동, 위상벽의 차단/통과 교대 등 종류별 상호작용이 있으며 각 벽은 최대 1개만 배치할 수 있습니다. 가짜벽은 제작자 화면을 포함한 모든 2D·3D 시점에서 정상벽과 완전히 같은 형상과 진한 적색으로 표시되고, 처음 충돌하기 전까지 무기한 유지됩니다.

최종 벽 비용은 가짜벽과 웜홀이 각각 7, 독벽이 3이며 나머지 함정·특수벽은 1입니다. 웜홀 제작자는 외부 입구와 복귀 지점만 고르며, 게임은 막힌 칸 1~3개가 있는 4×4 주사위 방을 자동 생성합니다. 주자는 방향키로 주사위를 굴려 출구의 목표 윗면을 맞춰야 외부로 돌아오며, 생성기는 좌표와 24가지 주사위 방향을 함께 검색해 최단 해법 6~8행동을 보장합니다. 웜홀 내부에서는 실패 횟수와 관계없이 해법 방향을 안내하지 않습니다. 모든 아이템은 종류별 1개로 제한되고 탐지기와 플레이어 스킬은 신규 맵 제작·저장과 공식 턴 처리에서 제거되었습니다.
