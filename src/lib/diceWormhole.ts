...
export const DICE_WORMHOLE_MIN_BLOCKED_CELLS = 1 as const;
export const DICE_WORMHOLE_MAX_BLOCKED_CELLS = 3 as const;
export const DICE_WORMHOLE_MIN_STEPS = 6 as const;
export const DICE_WORMHOLE_MAX_STEPS = 8 as const;
...
function getDiceChallengeShapeError(value: unknown): string | null {
  ...
  if (!isDensePositionArray(
    ...
    return `주사위 웜홀 장애물은 ${DICE_WORMHOLE_MIN_BLOCKED_CELLS}~${DICE_WORMHOLE_MAX_BLOCKED_CELLS}개여야 합니다.`;
  }
...
export function getDiceWormholeChallengeError(value: unknown): string | null {
  ...
  if (steps < DICE_WORMHOLE_MIN_STEPS || steps > DICE_WORMHOLE_MAX_STEPS) {
    return `주사위 웜홀 최단 동선은 ${DICE_WORMHOLE_MIN_STEPS}~${DICE_WORMHOLE_MAX_STEPS}행동이어야 합니다.`;
  }
  return null;
}
...
export const DICE_WORMHOLE_FALLBACK_CHALLENGE: Readonly<DiceWormholeChallenge> = Object.freeze({
  version: 2 as const,
  boardSize: DICE_WORMHOLE_BOARD_SIZE,
  startPosition: Object.freeze({ row: 0, col: 0 }),
  endPosition: Object.freeze({ row: 3, col: 3 }),
  blockedCells: Object.freeze([
    Object.freeze({ row: 1, col: 1 }),
    Object.freeze({ row: 2, col: 2 }),
  ]) as unknown as Position[],
  initialOrientation: 0,
  targetTop: 2 as DiceFace,
});
...
export function generateDiceWormholeChallenge(seed: number): DiceWormholeChallenge {
  ...
  for (let attempt = 0; attempt < 512; attempt += 1) {
    ...
    if (isValidDiceWormholeChallenge(candidate)) return candidate;
  }
